...
class ZCAN(object):
    def __init__(self, zlgcan_type):
        if platform.system() == "Windows":
            if zlgcan_type:
                self.__dll = windll.LoadLibrary("./zlgcan.dll")
            else:
                self.__dll = windll.LoadLibrary("./zlgcan_zq.dll")
        else:
    ...
    def ClearBuffer(self, chn_handle):
        try:
            # print(f"ZCAN_ClearBuffer Channel Handle: {chn_handle}")
            handle = ctypes.c_ulonglong(chn_handle)
    ...
    def TransmitFD(self, chn_handle, fd_msg, len):
        try:
            # 将 chn_handle 转换为 ctypes 对象，假设它是一个 64 位的整数
            # print(f"-----------------------------ZCAN_TransmitFD Channel Handle: {chn_handle}")
            handle = ctypes.c_ulonglong(chn_handle)
...
def can_start(zcanlib, device_handle, chn, arb_baud_rate, data_baud_rate):
    zcanlib.ZCAN_SetValue(device_handle, "0/canfd_standard", "0")  #
    zcanlib.ZCAN_SetValue(device_handle, "0/canfd_abit_baud_rate", arb_baud_rate)  # 设置仲裁域波特率
    zcanlib.ZCAN_SetValue(device_handle, "0/canfd_dbit_baud_rate", data_baud_rate)  # 设置数据域波特率
    zcanlib.ZCAN_SetValue(device_handle, "0/filter_clear", "0")  # 清除滤波

    # 终端电阻
    ret = zcanlib.ZCAN_SetValue(device_handle, str(chn) + "/initenal_resistance", "1")
