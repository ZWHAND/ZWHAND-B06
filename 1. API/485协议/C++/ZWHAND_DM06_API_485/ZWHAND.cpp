...
bool ZWHAND::set_single_motor_absolute_6dof(int motor_number, int joint_angle) {
    if (motor_number < 1 || motor_number > motor_count) {
...
bool ZWHAND::set_all_motor_absolute_6dof(const std::vector<int>& joint_angle_list) {
...
bool ZWHAND::set_single_motor_relative_6dof(int motor_number, int joint_angle) {
    if (motor_number < 1 || motor_number > motor_count) {
        std::cout << "设置电机角度失败: " << motor_number << " - 参数超出有效范围" << std::endl;
        return false;
    }
...
bool ZWHAND::set_all_motor_relative_6dof(const std::vector<int>& joint_angle_list) {
    if (static_cast<int>(joint_angle_list.size()) != motor_count) {
        std::cout << "设置所有电机角度失败: 数组长度不匹配!" << std::endl;
        return false;
    }
...
bool ZWHAND::set_single_motor_calibration_6dof(int motor_number) {
    if (motor_number < 1 || motor_number > motor_count) {
...
bool ZWHAND::set_all_motor_calibration_6dof() {
